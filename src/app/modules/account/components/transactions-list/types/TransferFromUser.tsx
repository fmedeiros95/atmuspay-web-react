...
const TransferFromUser = ({transaction}: any): JSX.Element => (
	<tr>
		<td style={{width: '36px'}}>
		<img src={userAvatar(transaction?.transfer?.send_from?.avatar)} alt="user-avatar" className="rounded avatar-sm" />
		</td>
		<td>
			...
			<p className="mb-0 text-muted">
				<small>{formatCpfCnpj(transaction?.transfer?.send_from?.document?.document)}</small>
			</p>
		</td>
